...
This skill should be used when working with Serena's memory system: when the user asks to "create a memory", "read memories", "manage memories", or when starting work on a project with existing memories. Essential for maintaining project knowledge across multiple conversation sessions in Serena.
