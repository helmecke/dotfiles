...
Comprehensive guidance on creating OpenCode slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
OpenCode activates this skill when users:
...
OpenCode loads references and examples as needed based on task.
...
- **Project**: `.opencode/command/` (shared with team)
- **Personal**: `~/.config/opencode/command/` (your commands)
- **Plugin**: `plugin-name/command/` (plugin-specific)
...
| `model` | Specify model | `anthropic/claude-3-5-sonnet-20241022` |
| `argument-hint` | Document arguments | `[pr-number] [priority]` |
| `agent` | Specify agent | `build`, `plan` |
| `subtask` | Execute as subagent | `true` |
...
6. Test examples work with current OpenCode

## Version History

**v0.3.0** (2025-12-23):
- **CRITICAL:** Fixed bash execution syntax from `!`cmd`` to `*!`cmd`*`
- **CRITICAL:** Fixed command file paths from `.claude/commands/` to `.opencode/command/`
- **CRITICAL:** Updated product name from "Claude Code" to "OpenCode"
- Added JSON configuration documentation and examples
- Added `agent` and `subtask` frontmatter field documentation
- Updated model field to use full identifiers (e.g., `anthropic/claude-3-5-sonnet-20241022`)
- Updated all 20 examples with correct syntax
- Updated all path references throughout skill

**v0.2.0** (2025-01-16):
- Added plugin-specific features and patterns
- Added validation patterns
- Added integration with agents, skills, and hooks
